...
apihelper.proxy = {'https': 'socks5://127.0.0.1:51261'}
bot = telebot.TeleBot('7546088228:AAGZ6EIBS6gMJI4oIPL6MvPTBQKkkTQxBlQ')
...
def summa(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, 'Неверный формат. Впишите сумму заново')
        bot.register_next_step_handler(message, summa)
        return
    if amount > 0:
        ...
        btn4 = types.InlineKeyboardButton('Свой вариант', callback_data='else')
...
# Обрабатываем callback_data что выбрал пользователь какую валюту
...
def my_currency(message):
    try:
        ...
        bot.send_message(message.chat.id, 'Вы ввели неверные значения. Введите заново')
